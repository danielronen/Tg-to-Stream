...
def find_video_collection():
    ...
    possible_names = ['files', 'media', 'messages', 'telegram_files', 'tg_files', 
...
def get_all_videos(skip=0, limit=100):
    ...
    try:
        ...
        files = list(collection.find({"msg_id": {"$exists": True, "$ne": None}})
...
async def meta(request):
    ...
    video_doc = collection.find_one({"msg_id": video_id}) or collection.find_one({"msg_id": int(video_id) if video_id.isdigit() else None})
    
    if not video_doc:
        return JSONResponse({"meta": {}})
        
    return JSONResponse({"meta": video_to_meta(video_doc)}, headers={'Access-Control-Allow-Origin': '*'})
...
async def catalog(request):
    ...
    try:
        ...
        if search_query:
            ...
            cursor = collection.find({"msg_id": {"$exists": True}}).sort("_id", -1).skip(skip).limit(100)
